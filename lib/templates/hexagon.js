...
getTemplateHead = (size, dimensions = DEFAULT_DIMENSIONS) => {
...
const makeTemplate = ({
  ...
  const start = startColor
    ? utils.convertHexToRgb(startColor)
    : DEFAULT_START_COLOUR;
  const end = endColor
    ? utils.convertHexToRgb(endColor)
    : DEFAULT_END_COLOUR;
  ...
  const colorMatrix = utils.createColorMatrix(start, end, shapeSize, noise, [width, height]);
